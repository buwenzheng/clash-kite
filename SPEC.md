...
> 本节是产品边界的"硬约束"，优先级高于后续所有章节。

### 0.1 项目定位

Clash-Kite 是一款**轻量、够用**的 mihomo 桌面客户端，**专注于交付 mihomo 的标准功能集**，不做差异化集成。

### 0.2 范围内（In Scope）✅

- mihomo 内核管理（启动/停止/重启/版本切换）
- 代理基础：开关、模式切换（rule/global/direct）、节点选择、节点测速
- 配置管理：本地文件 / 订阅 URL 导入、切换、删除、导出、YAML 编辑
- 连接管理：实时查看、关闭单条、关闭全部
- 日志查看（轮询 / 实时流）
- mihomo 高级配置：SysProxy 高级、TUN、DNS、Sniffer、Kernel、Resources（GeoIP/Provider）
- 体验增强：托盘菜单、二维码导入、订阅自动更新、开机自启、工作目录、规则查看
- 国际化（zh/en）、深浅主题、托盘
...
| 不做项 | 原因 |
|--------|------|
| SmartCore / AI 节点选择 | 与"轻量"定位冲突 |
| Sub-Store 集成 | 自研订阅管理已够用 |
| 双内核（Smart 内核 + Mihomo） | 仅内置 mihomo |
| TUN 免服务模式（无管理员权限） | 需要 Win 服务模式 / macOS root，门槛高 |
| **Overrides 覆写系统**（YAML/JS 覆写） | 用户可通过 YAML 编辑器直接改 |
| **WebDAV 备份/恢复** | 非核心需求 |
| 应用自动更新 | GitHub Release 检查 + 升级复杂 |
| 多配色主题 | 仅保留深浅模式 |
| MetaCubeXd Dashboard 内置 | 用户可自行启用 mihomo 外部控制器 |

> **任何新增功能必须先更新本节**，经评审后纳入 In Scope。
...
- **轻量优先**：Tauri 框架，安装包 < 20 MB，内存 < 100 MB
- **简单易用**：一键连接，标准 mihomo 体验
- **开源免费**：无广告，无追踪
- **跨平台**：Windows / macOS 统一体验

### 非目标（Non-Goals）

- ❌ 不与 clash-party / FlClash 在差异化功能上竞争
- ❌ 不做覆写、备份、AI、Sub-Store 等高阶集成
- ❌ 不追求"开箱即用免服务模式 TUN"（需要权限提升，对轻量用户不友好）
...
共 **13 个页面**（已去除 Overrides 页），按优先级分组：
...
| Logs | `/logs` | P0 | ✅ 已完成 |
| Connections | `/connections` | P0 | ✅ 已完成 |
...
| Rules | `/rules` | P2 | 📋 待设计（仅查看与启用/禁用，不做覆写） |

> **注**：原 Overrides 页（ `/overrides`）已从范围中移除（见 §0.3）。
...
显示当前生效的规则列表，支持搜索、过滤、启用/禁用（**仅查看与轻量编辑，不做覆写引擎**）。
...
> **显式移除**：Connections 主命令（已完成， 5 个）、Overrides 相关、MetaCubeXd Dashboard、应用自动更新、WebDAV。
...
| T-02 | SysProxy 高级配置 | P1 | — | 📋 |
| T-03 | TUN 配置 | P1 | T-02 | 📋 |
| T-04 | DNS 配置 | P1 | T-02 | 📋 |
| T-05 | Sniffer 配置 | P1 | T-02 | 📋 |
| T-06 | Resources（GeoIP/Provider）| P1 | T-02, T-03 | 📋 |
| T-07 | Kernel 配置 + 版本管理 | P1 | T-02 | 📋 |
...
| T-12 | 开机自启 | P1 | — | 📋 |
| T-13 | 工作目录配置与迁移 | P1 | — | 📋 |
| T-14 | Rules 查看页 | P2 | T-04~T-07 | 📋 |
| T-15 | 全局快捷键 | P2 | — | 📋 |
| T-16 | 备份/迁移（压缩包）| P2 | — | 📋 |

> **已移除的任务**：T-11 Overrides 覆写系统、T-13 MetaCubeXd Dashboard、T-18 应用自动更新、T-19 WebDAV 同步（原编号）——见 §0.3 范围外说明。
...
| 2026-06-03 | 范围重定义：明确不做 SmartCore/Sub-Store/双内核/Overrides/WebDAV/应用自动更新/多主题/免服务 TUN；移除 P2 Overrides 页面；任务看板重新编号 |
