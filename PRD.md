...
| Profiles | `/profiles` | P0 | [docs/profile-module.md](docs/profile-module.md) | 订阅导入/更新/编辑/激活/二维码导入；依赖本地 profile 文件与热重载 |
...
| Kernel | `/kernel` | P1 | [docs/kernel-module.md](docs/kernel-module.md) | `get_kernel_settings/set_kernel_settings/apply_kernel_settings`；进程优先级（Windows）、内核版本管理；可用 `POST /restart` 兜底 |
| DNS | `/dns` | P1 | [docs/dns-module.md](docs/dns-module.md) | `get_dns_config/set_dns_config/apply_dns_config`；enhanced-mode、nameserver、fallback、fake-ip |
| Sniffer | `/sniffer` | P1 | [docs/sniffer-module.md](docs/sniffer-module.md) | `get_sniffer_config/set_sniffer_config/apply_sniffer_config`；协议嗅探（HTTP/TLS/QUIC）、force/skip-domain |
| Settings | `/settings` | P0 | [docs/settings-module.md](docs/settings-module.md) | `get_settings/save_settings`；托盘基础逻辑依赖代理状态；工作目录配置 |
...
| Profiles    | `/profiles`    | 订阅/本地/二维码导入、编辑、YAML 编辑器、导出、激活删除     | P0     |
...
| DNS         | `/dns`         | DNS 开关、enhanced-mode、nameserver/fallback、fake-ip 配置  | P1     |
| Sniffer     | `/sniffer`     | 嗅探开关、协议配置（HTTP/TLS/QUIC）、force/skip-domain      | P1     |
| Resources   | `/resources`   | GeoIP/GeoSite/ASN 管理、Proxy Provider、Rule Provider       | P1     |
| Kernel      | `/kernel`      | 端口配置、日志级别、allow-lan、IPv6、外部控制器、进程优先级、内核版本管理 | P1     |
| Settings    | `/settings`    | 主题切换、语言、托盘设置、工作目录配置、MetaCubeXd Dashboard、关于信息 | P0     |
...
- [ ] DNS 配置页面（enhanced-mode、nameserver/fallback、fake-ip）
- [ ] Sniffer 域名嗅探页面（协议嗅探 HTTP/TLS/QUIC、force/skip-domain）
- [ ] 外部资源页面（GeoIP/GeoSite/ASN 管理、Proxy/Rule Provider）
- [ ] 内核设置页面（端口、日志级别、allow-lan、IPv6、外部控制器、高级选项）
- [ ] 内核版本管理（mihomo 稳定版/预览版切换、GitHub 自动下载）
- [ ] mihomo 进程优先级设置（仅 Windows，CPU 优先级调整）
- [ ] 二维码导入订阅（截屏识别 / 图片文件解析 QR Code）
- [ ] 托盘菜单增强（模式切换、系统代理开关、配置切换子菜单）
- [ ] 托盘 P1 功能（代理组节点选择、图标颜色状态、打开目录、重启应用）
- [ ] 订阅自动更新（每个订阅独立间隔，默认 8h，激活配置自动热重载，系统通知）
- [ ] 工作目录配置（自定义数据存储目录、数据迁移）
- [ ] MetaCubeXd Dashboard 集成（内置 mihomo 官方 Web Dashboard 访问入口）
- [ ] WebDAV 配置同步
- [ ] TUN 模式支持
- [ ] 开机自启功能
...
| [docs/profile-module.md](docs/profile-module.md) | 配置管理模块详细设计（导入/导出/编辑/激活/二维码导入）|
| [docs/settings-module.md](docs/settings-module.md) | 设置与系统集成模块（主题/国际化/托盘/工作目录/Dashboard） |
...
| [docs/dns-module.md](docs/dns-module.md) | DNS 配置模块详细设计（enhanced-mode、nameserver、fallback、fake-ip） |
| [docs/sniffer-module.md](docs/sniffer-module.md) | 域名嗅探模块详细设计（协议嗅探 HTTP/TLS/QUIC、force/skip-domain） |
...
| [docs/kernel-module.md](docs/kernel-module.md) | 内核设置模块详细设计（端口、进程优先级、内核版本管理） |
...
| fake-ip   | DNS 增强模式，返回假 IP 用于规则匹配       |
| Sniffer   | 域名嗅探器，从握手包中还原真实域名         |
...
- [clash-party 项目](https://github.com/mihomo-party-org/clash-party)
...
- [MetaCubeXd Dashboard](https://github.com/MetaCubeX/metacubexd)
...
| 2026-03-20 | v2.9.0 | 对标 clash-party：新增 DNS 配置页面、Sniffer 域名嗅探页面、mihomo 进程优先级、内核版本管理、二维码导入订阅、MetaCubeXd Dashboard 集成、工作目录配置 |
