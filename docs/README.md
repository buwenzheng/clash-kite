# Clash-Kite 文档

欢迎查阅 Clash-Kite 文档！

## 文档列表

### 使用文档

- [使用指南 (中文)](./USAGE.md) - 详细的使用说明
- [User Guide (English)](./USAGE_EN.md) - Detailed usage instructions

### 开发文档

- [开发文档](./DEVELOPMENT.md) - 开发环境搭建和架构说明

### 模块文档
- [代理模块（proxy-module）](./proxy-module.md)
- [配置管理模块（profile-module）](./profile-module.md)
- [设置与系统集成模块（settings-module）](./settings-module.md)
- [系统代理模块（sysproxy-module）](./sysproxy-module.md)
- [TUN 虚拟网卡模块（tun-module）](./tun-module.md)
- [外部资源模块（resources-module）](./resources-module.md)
- [日志模块（logs-module）](./logs-module.md)
- [连接模块（connections-module）](./connections-module.md)
- [内核设置模块（kernel-module）](./kernel-module.md)
- [规则与覆写模块（rules-module）](./rules-module.md)

## 快速链接

- [GitHub 仓库](https://github.com/buwenzheng/clash-kite)
- [问题反馈](https://github.com/buwenzheng/clash-kite/issues)

## 文档贡献

如果发现文档有误或需要补充，欢迎提交 Pull Request。
