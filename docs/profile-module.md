...
> 参考 clash-party，通过扫描或截图识别二维码导入订阅链接。
...
用户点击"二维码导入"按钮
  ...
  ▼
弹出 Dialog，提供两种方式：
  ├─ 截屏识别：调用 Tauri 截屏 API → 检测二维码
  └─ 图片文件：选择本地图片文件 → 解析二维码
  │
  ▼
解析出 URL → 自动填入订阅 URL 输入框
  → 走标准订阅导入流程 (import_profile_subscription)
```

**技术方案：**
- Rust 端使用 `image` + `rqrr` crate（或 `bardecoder`）解析二维码
- 支持从屏幕截图和本地图片文件两种输入方式
- 解析成功后自动填入订阅 URL，用户确认后执行导入

**Tauri Command 契约：**

| Command                | 参数                        | 返回值                | 说明                    |
| ---------------------- | --------------------------- | --------------------- | ----------------------- |
| `decode_qr_from_screen`| 无                          | `String \| null`      | 截屏并识别二维码        |
| `decode_qr_from_file`  | `file_path: String`         | `String \| null`      | 从图片文件识别二维码    |
